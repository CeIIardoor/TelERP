<template>
  <div>
    ...
    <trashed-message v-if="organisation.deleted_at" class="mb-6" @restore="restore"> This organisation has been deleted. </trashed-message>
    ...
    <h2 class="mt-12 text-2xl font-bold">Collaberateurs</h2>
...
export default {
  ...
  data() {
    return {
      form: this.$inertia.form({
        ...
        region: this.organisation.region,
  ...
  methods: {
    update() {
      this.form.put(`/organisations/${this.organisation.id}`)
    },
    destroy() {
      if (confirm('Are you sure you want to delete this organisation?')) {
        this.$inertia.delete(`/organisations/${this.organisation.id}`)
      }
    },
    restore() {
      if (confirm('Are you sure you want to restore this organisation?')) {
        this.$inertia.put(`/organisations/${this.organisation.id}/restore`)
      }
    },
  },
}
